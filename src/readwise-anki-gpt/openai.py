    ...
    with sqlite3.connect(config.READWISE_SQLITE_DATABASE_PATH) as conn:
        ...
        try:
            with ThreadPoolExecutor() as executor:
                futures = {
                    executor.submit(
                        generate_card, openai_client, highlight_record
                    ): highlight_record
                    for highlight_record in limited_records
                }
                for future in as_completed(futures):
                    try:
                        ...
                        message_box_signal.show_message.emit(
...
def generate_card(openai_client: OpenAI, highlight_record: dict) -> dict:
    ...
    try:
        ...
        response_json_array = json.loads(
            choice.message.content
        )  # TODO: Add exception handling if completion does not return valid JSON
    except json.JSONDecodeError:
        message_box_signal.show_message.emit(
            QMessageBox.Icon.Warning,
            "JSON Decode Error",
            f"{json.dumps(choice, indent=4)}<br><br>{traceback.format_exc()}",
            QMessageBox.StandardButton.Ok,
        )
